...
const code = fs.readFileSync('src/app/insurer/page.tsx', 'utf8');

// I will remove the injected block first.
const badBlockStart = "{/* Main Content */}\n\n        {tab === 'AUDIT' && (";
const badBlockRegex = /\{\/\* Main Content \*\/\}\n\n        \{tab === 'AUDIT' && \([\s\S]*?Measuring performance telemetry\.\.\.\<\/div\>\n            \)\}\n          \<\/div\>\n        \)\}\n/m;

const cleanedCode = code.replace(badBlockRegex, "{/* Main Content */}\n");

// Now I will inject the tabs INSIDE the main content div, right after it opens
const targetPoint = `<div className="flex-1 p-8 overflow-y-auto">`;
const split = cleanedCode.split(targetPoint);
if (split.length === 2) {
  const finalCode = split[0] + targetPoint + "\n" + `
        {tab === 'AUDIT' && (
...
` + split[1];
  fs.writeFileSync('src/app/insurer/page.tsx', finalCode);
} else {
  console.log("Failed to inject correctly");
}
